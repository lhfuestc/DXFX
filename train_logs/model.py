...
def fcn(inputs, maps_s1, maps_s2, maps_s3, dropprob):
    std = 0.1
    mean = 0.0
    dtype = tf.float32
...
def Residual_Map_Block(inputs, outMaps, kSize, wDecay = 0.0, nLayers = 1, name = None):
    assert len(inputs.get_shape()) == 4, "Previous layer must be 4D [b, h, w, c]!"
    
    by_pass = inputs
    kShape = [kSize, kSize, outMaps, outMaps]
    for i in range(nLayers):
        inputs = conv2d_layer_no_act(inputs, kShape, 1, wDecay, (name + "%d_conv/" % i))
        inputs = activation(inputs, types = 'relu', name = name + ("%d_relu/" % i))
        inputs = conv2d_layer_no_act(inputs, kShape, 1, wDecay, (name + "conv_%d/" % i))
        inputs = 0.1*inputs
    return inputs + by_pass
...
def Res_Block_BN(inputs, nlayer, wdecay = 0.0, name = None):
    ...
    for i in range(nlayer):
        ...
        inputs = conv2d_layer_no_act(inputs, Kshape, stride,  wdecay, name = (name + ('conv2d_layer_1_%d' % i))) 
        inputs = activation(inputs, types = 'relu', name = (name + ('relu_1_%d' % i)))
        inputs = conv2d_layer_no_act(inputs, Kshape, stride,  wdecay, name = (name + ('conv2d_layer_2_%d' % i))) 
...
def Res_Net(inputs, drop):
    ...
    inputs = pooling(inputs, ksize_m, stride_m, pool_mode, 'maxpool_0')
    for i in range(3):
        inputs = Res_Block_BN(inputs, nlayer, wdecay = 0.0, name = ('resbk1_%d/' % i))

    inputs = pooling(inputs, ksize_m, stride_m, pool_mode, 'maxpool_1')
    
    for i in range(5):
        inputs = Res_Block_BN(inputs, nlayer, wdecay = 0.0, name = ('resbk2_%d/' % i))
        
    inputs = pooling(inputs, ksize_m, stride_m, pool_mode, 'maxpool_2')
    
    for i in range(5):
        inputs = Res_Block_BN(inputs, nlayer, wdecay = 0.0, name = ('resbk3_%d/' % i))
    
    inputs = pooling(inputs, ksize_m, stride_m, pool_mode, 'maxpool_3')
    
    for i in range(7):
        inputs = Res_Block_BN(inputs, nlayer, wdecay = 0.0, name = ('resbk4_%d/' % i))
    
    inputs = pooling(inputs, ksize_m, stride_m, pool_mode, 'maxpool_4')
    
    inputs = fcn(inputs, 1024, 1024, 5, drop)
    
    #inputs = inputs + inp_mean
    return inputs    


def fcn_2(inputs, drop, maps_s1, maps_s2):
    std = 0.001
    mean = 0.0
    dtype = tf.float32
    c, w, d = inputs.get_shape().as_list()[1:4]
   ...
   # dropprob = drop if isTraining else 1
    ...
    fc1_drop = tf.nn.dropout(fc1, drop, name = "fc1_drop")  
...
def My_Model(inputs, Drop):
    ...
    with tf.variable_scope("CONV_BLOCK_3b"):
        ...
        with tf.variable_scope("Inception_Block"):
            ...
            inputs_1 = tf.concat([inputs_3b_1, inputs_3b_2, inputs_3b_3, inputs_3b_4], 3)
            inputs_1 = fcn_2(inputs_1, Drop, 1024, 5)
    ...
    with tf.variable_scope("CONV_BLOCK_4e"):
        ...
        with tf.variable_scope("Inception_Block"):
            ...
            inputs_2 = fcn_2(inputs_2, Drop, 1024, 5)
    ...
    with tf.variable_scope("CONV_BLOCK_5b"):
        ...
        with tf.variable_scope("Inception_Block"):
            ...
            inputs_3 = fcn_2(inputs_3, Drop, 512, 5)
    #inputs = inputs_1*0.1 + inputs_2*0.3  + inputs_3*0.3
    inputs = inputs_3 
    return inputs
    
   
def my_VGG(inputs, drop):
    assert len(inputs.shape) == 4, "The dimension of inputs should be 4!"
    kernel_size_m = 2
    conv_stride_m = 2
    ...
    conv_stride = 1
    pool_mode = 'max'
    inputs = pooling(inputs, kernel_size_m, conv_stride_m, pool_mode, "maxpool0")
    #inputs = tf.space_to_depth(inputs, 2)
    in_maps = int(inputs.get_shape()[3])
    ...
    inputs = fcn(inputs, 512, 512, 5, drop)
    
    return inputs    
    
    
def My_Model_1(inputs, Drop):
    assert len(inputs.shape) == 4, "The dimension of inputs should be 4!"
    
    with tf.variable_scope("CONV_BLOCK_1"):
        in_maps = int(inputs.get_shape()[3])
        kshape = [3, 3, in_maps, 64]
        inputs = conv2d_layer(inputs, kshape, 2, WEIGHT_DECAY, 'conv2d_layer_1')
        inputs = pooling(inputs, 2, 2, 'max', 'maxpooling_layer')
    ...
    with tf.variable_scope("CONV_BLOCK_3a"):
        in_maps = int(inputs.get_shape()[3])
        inputs_3a = pooling(inputs, 2, 2, 'max', 'maxpooling_layer')
        with tf.variable_scope("Inception_Block"):
            kshape = [3, 3, in_maps, 64]
            inputs_3a_1 = conv2d_layer(inputs_3a, kshape, 1, WEIGHT_DECAY, 'conv2d_layer_1')
            
            kshape = [3, 3, in_maps, 96]
            inputs_3a_2 = conv2d_layer(inputs_3a, kshape, 1, WEIGHT_DECAY, 'conv2d_layer_2')
            kshape = [3, 3, 96, 128]
            inputs_3a_2 = conv2d_layer(inputs_3a_2, kshape, 1, WEIGHT_DECAY, 'conv2d_layer_3')
            
            kshape = [3, 3, in_maps, 16]
            inputs_3a_3 = conv2d_layer(inputs_3a, kshape, 1, WEIGHT_DECAY, 'conv2d_layer_4')
            kshape = [5, 5, 16, 32]
            inputs_3a_3 = conv2d_layer(inputs_3a_3, kshape, 1, WEIGHT_DECAY, 'conv2d_layer_5')
            
            inputs_3a_4 = pooling(inputs_3a, 3, 1, 'max', 'maxpooling_layer')
            kshape = [3, 3, in_maps, 32]
            inputs_3a_4 = conv2d_layer(inputs_3a_4, kshape, 1, WEIGHT_DECAY, 'conv2d_layer_6')
            
            inputs = tf.concat([inputs_3a_1, inputs_3a_2, inputs_3a_3, inputs_3a_4], 3)
            #inputs_1 = fcn_2(inputs, Drop, 512, 5)
    with tf.variable_scope("CONV_BLOCK_3b"):
        in_maps = int(inputs.get_shape()[3])
        inputs_3b = pooling(inputs, 2, 2, 'max', 'maxpooling_layer')
        with tf.variable_scope("Inception_Block"):
            kshape = [3, 3, in_maps, 128]
            inputs_3b_1 = conv2d_layer(inputs_3b, kshape, 1, WEIGHT_DECAY, 'conv2d_layer_1')
            ...
            #inputs_2 = fcn_2(inputs, Drop, 512, 5)
    with tf.variable_scope("CONV_BLOCK_3c"):
        in_maps = int(inputs.get_shape()[3])
        inputs_3c = pooling(inputs, 2, 2, 'max', 'maxpooling_layer')
        with tf.variable_scope("Inception_Block"):
            kshape = [3, 3, in_maps, 192]
            inputs_3c_1 = conv2d_layer(inputs_3c, kshape, 1, WEIGHT_DECAY, 'conv2d_layer_1')
            
            kshape = [3, 3, in_maps, 96]
            inputs_3c_2 = conv2d_layer(inputs_3c, kshape, 1, WEIGHT_DECAY, 'conv2d_layer_2')
            kshape = [3, 3, 96, 208]
            inputs_3c_2 = conv2d_layer(inputs_3c_2, kshape, 1, WEIGHT_DECAY, 'conv2d_layer_3')
            
            kshape = [3, 3, in_maps, 16]
            inputs_3c_3 = conv2d_layer(inputs_3c, kshape, 1, WEIGHT_DECAY, 'conv2d_layer_4')
            kshape = [5, 5, 16, 48]
            inputs_3c_3 = conv2d_layer(inputs_3c_3, kshape, 1, WEIGHT_DECAY, 'conv2d_layer_5')
            
            inputs_3c_4 = pooling(inputs_3c, 3, 1, 'max', 'maxpooling_layer')
            kshape = [3, 3, in_maps, 64]
            inputs_3c_4 = conv2d_layer(inputs_3c_4, kshape, 1, WEIGHT_DECAY, 'conv2d_layer_6')
            
            inputs = tf.concat([inputs_3c_1, inputs_3c_2, inputs_3c_3, inputs_3c_4], 3)

            c = inputs.get_shape().as_list()[1]
            inputs_3 = pooling(inputs, c, 1, 'avg', 'avgpooling_layer')
            inputs_3 = fcn_2(inputs_3, Drop, 512, 5)
    inputs = inputs_3
    #inputs = inputs_1 * 0.3 + inputs_3 * 0.7
    return inputs   
    

def Edsr_Model(inputs):
    num_maps = 32
    num_blocks = 8
    kSize = 3
    wDecay = 0.0
    inputs = inputs - 0.5
    num_channs = int(inputs.get_shape()[3])
    external_shortcut = inputs
    kShape = [kSize, kSize, num_channs, num_maps]
    inputs = conv2d_layer_no_act(inputs, kShape, 1, wDecay, "init_conv")
    
    by_pass = inputs
    for i in range(num_blocks):
        inputs = Residual_Map_Block(inputs, num_maps, kSize, name = ('rb%d/' % i))
    kShape = [kSize, kSize, num_maps, num_maps]
    inputs = conv2d_layer_no_act(inputs, kShape, 1, wDecay, "post_conv")
    
    inputs = by_pass + inputs  
    kShape = [kSize, kSize, num_maps, num_channs]
    inputs = conv2d_layer_no_act(inputs, kShape, 1, wDecay, "last_conv")
    
    inputs = inputs + external_shortcut
    inputs = inputs + 0.5
    inputs = activation(inputs, types = 'relu01', name = "last_relu01/")
    return inputs


def RNCNN(inputs, Drop):
    output_image = Edsr_Model(inputs)
    output_label = my_VGG(inputs, Drop)
    return output_image, output_label


def my_VGG_1(inputs, drop):
    assert len(inputs.shape) == 4, "The dimension of inputs should be 4!"
    kernel_size_m = 2
    conv_stride_m = 2
