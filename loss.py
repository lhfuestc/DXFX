...
def loss_cross(images, labels): 
    with tf.name_scope('cross_loss'):
        cross_entropy = tf.nn.sparse_softmax_cross_entropy_with_logits(logits = images, labels = labels, name='cross_entropy_per_example')
        cross_entropy_mean = tf.reduce_mean(cross_entropy, name='cross_entropy')
        tf.add_to_collection(TOTAL_LOSS_COLLECTION, cross_entropy_mean )
        tf.summary.scalar('loss', cross_entropy_mean)
    return cross_entropy_mean


def loss_all(clear_imgs, pred_imgs, pred_labels, labels):
    with tf.name_scope('Loss'):
        abs_loss = tf.reduce_mean(tf.abs(clear_imgs - pred_imgs), name = 'L1_LOSS')
        tf.add_to_collection(TOTAL_LOSS_COLLECTION, abs_loss)
        cross_entropy = tf.nn.sparse_softmax_cross_entropy_with_logits(logits = pred_labels, labels = labels, name='cross_entropy_per_example')
        cross_entropy_mean = tf.reduce_mean(cross_entropy, name='cross_entropy')
        tf.add_to_collection(TOTAL_LOSS_COLLECTION, cross_entropy_mean)
    return tf.add_n(tf.get_collection(TOTAL_LOSS_COLLECTION))
