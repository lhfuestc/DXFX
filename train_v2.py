...
def augmentation(h_patch, l_patch):
    ...
    if rot == 2:    # up-down flip
        ...
        H_Patch = np.transpose(h_patch, [1, 0, 2])
        L_Patch = np.transpose(l_patch, [1, 0, 2])
    elif rot == 6:  # transpose + up-down flip
        H_Patch = np.transpose(h_patch, [1, 0, 2])[::-1, :, :]
        L_Patch = np.transpose(l_patch, [1, 0, 2])[::-1, :, :]
    elif rot == 7:  # transpose + left-right flip
        H_Patch = np.transpose(h_patch, [1, 0, 2])[:, ::-1, :]
        L_Patch = np.transpose(l_patch, [1, 0, 2])[:, ::-1, :]
    else:           # transpose + up-down + left-right flip
        H_Patch= np.transpose(h_patch, [1, 0, 2])[::-1, ::-1, :]
        L_Patch= np.transpose(l_patch, [1, 0, 2])[::-1, ::-1, :]
    return H_Patch, L_Patch


def Validation(pred_labels, h_labels, l_labels):
    #prediction_labels = np.argmax(pred_labels, axis=1)
    #num = len(labels)
    #labels = np.reshape(labels, (num, 1))
    assert (h_labels == l_labels).all(), "h_labels and l_labels must be equal"
    correct_prediction = np.equal(pred_labels, h_labels)
...
def Validation_1(pred_labels, h_labels):
    #prediction_labels = np.argmax(pred_labels, axis=1)
    #num = len(labels)
    #labels = np.reshape(labels, (num, 1))
    correct_prediction = np.equal(pred_labels, h_labels)
...
def get_train_batch(train_h_images, train_h_labels, train_l_images, train_l_labels, img_index, batch_size):
    train_h_img_batch = np.zeros([batch_size, img_H, img_W, channel], dtype = np.float32)
    train_h_lab_batch = np.zeros([batch_size], dtype = np.int64)
    train_l_img_batch = np.zeros([batch_size, img_H, img_W, channel], dtype = np.float32)
    train_l_lab_batch = np.zeros([batch_size], dtype = np.int64)
    for i in range(batch_size):
        train_h_img, train_l_img = augmentation(train_h_images[img_index[i]],train_l_images[img_index[i]])
        train_h_img_batch[i, :, :, :] = train_h_img
        train_h_lab_batch[i] = train_h_labels[img_index[i]]
        train_l_img_batch[i, :, :, :] = train_l_img
        train_l_lab_batch[i] = train_l_labels[img_index[i]]
    return train_h_img_batch, train_h_lab_batch, train_l_img_batch, train_l_lab_batch
...
def train_network():
    ...
    Img_H_Batch = tf.placeholder(dtype = tf.float32, shape = [None, img_H, img_W, channel] , name = 'input_h')
    Img_L_Batch = tf.placeholder(dtype = tf.float32, shape = [None, img_H, img_W, channel] , name = 'input_l')
    ...
    pred_images, pred_labels = model.RNCNN(Img_L_Batch, Drop)
    #pred_model = model_v1.My_Model(Img_Batch, Drop)
    #pred_model = model_v1.My_Model(Img_Batch, Drop)
    #pred_model = model.my_VGG(Img_Batch, Drop)
    finaloutput = tf.nn.softmax(pred_labels, name = 'softmax')
    prediction_labels = tf.argmax(finaloutput, axis=1, name = 'output')
    train_loss = loss.loss_all(Img_H_Batch, pred_images, pred_labels, Lab_Batch)
    ...
    train_h_images, train_h_labels, valid_h_images, valid_h_labels = Generate_Data(Data_H_Dir)
    train_l_images, train_l_labels, valid_l_images, valid_l_labels = Generate_Data(Data_L_Dir)
    
    print(termcolor.colored("Starting To Train...", 'green', attrs = ['bold']))
    num_images = len(train_h_images)
    ...
    for i in range(EPOCH):
        ...
        for k in index:
            ...
            train_h_img_batch, train_h_lab_batch, train_l_img_batch, train_l_lab_batch  = get_train_batch(train_h_images, train_h_labels, train_l_images, train_l_labels, index_img[k:k+Batch_Size], Batch_Size)
           
            duration1 = time.time() - start_time
            
            feed_dict = {Img_H_Batch:train_h_img_batch, Img_L_Batch:train_l_img_batch, Lab_Batch:train_h_lab_batch, Drop:0.5}
            _, model_loss, pred_train_labels = sess.run([train_op, train_loss, prediction_labels], feed_dict = feed_dict)
            duration2 = time.time() - start_time
            accuracy, correct_times_in_batch = Validation(pred_train_labels, train_h_lab_batch, train_l_lab_batch)
            if (step + 1) % 100 == 0:
            ...
            if (step == 0) or ((step + 1) % 100 == 0) or ((step + 1) == max_steps):
                ...
                valid_image_batch, valid_label_batch = get_valid_batch(valid_l_images, valid_l_labels)
        
                pred_valid_labels = sess.run(prediction_labels, feed_dict = {Img_L_Batch:valid_image_batch, Drop:1})
                
                accuracy, correct_times_in_batch = Validation_1(pred_valid_labels, valid_label_batch)
