...
def Generate_Data(data_dir):
    ...
    for i in range(num):
        ...
        for data in TFR:
            ...
            data = (np.reshape(data, (256, 256, 1)) + 1) * 0.5
            label = label
            images.append(data)
            labels.append(label)
...
def Image_Enhancement(image_list, label_list, Img_Dir_E, Img_Dir_R):
    num_image = len(image_list)
    with tf.Graph().as_default():
        output_graph_def = tf.GraphDef()

        with open(pb_file_path_IE, "rb") as f:
        ...
        with tf.Session() as sess:
            ...
            for i in range(num_image):
                label = label_list[i]
                single_image = np.reshape(image_list[i], (1, 256, 256, 1))
                image_E = sess.run(out_label, feed_dict={inputs:single_image})
                path_1 = os.path.join(Img_Dir_E, str(i) + '_' + str(label) + '.png')
                path_2 = os.path.join(Img_Dir_R, str(i) + '_' + str(label) + '.png')
                misc.imsave(path_1, image_E[0, :, :, 0])
                misc.imsave(path_2, single_image[0, :, :, 0])
...
def read_images(DIR):
    ...
    num_img = len(image_name_list)
    for i in range(num_img):
        path = os.path.join(DIR, image_name_list[i])
        im = misc.imread(path) / norm
        lab = int(image_name_list[i].split('_')[1].split('.')[0])
        label = lab - 1
        image = np.reshape(im, (256, 256, 1))
        image_list.append(image)
        label_list.append(label)
    train_img_list = image_list[:-200]
    train_lab_list = label_list[:-200]
    valid_img_list = image_list[-200:]
    valid_lab_list = label_list[-200:]
...
def get_train_batch(train_images, train_labels, img_index, batch_size):
    ...
    for i in range(batch_size):
        ...
        train_img = train_images[img_index[i]]
...
def train_network():
    ...
    pred_model = model.My_Model(Img_Batch, Drop)
    ...
    train_images, train_labels, valid_images, valid_labels = read_images(Data_Dir_E)
    ...
    for i in range(EPOCH):
        ...
        for k in index:
            ...
            if (step == 0) or ((step + 1) % 100 == 0) or ((step + 1) == max_steps):
                ...
                pred_valid_labels = sess.run(prediction_labels, feed_dict = {Img_Batch:valid_image_batch, Drop:1.0})
...
def main(argv = None):  
    if not train_from_exist:
        ...
        if not tf.gfile.Exists(train_log_dir):
            tf.gfile.MakeDirs(train_log_dir)
    #image_list, label_list = Generate_Data(Data_Dir)
    #Image_Enhancement(image_list, label_list, Data_Dir_E, Data_Dir_R)
    train_network()
